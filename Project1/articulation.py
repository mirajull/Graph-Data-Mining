...
def articulations(g, usegraphframe=False):
	...
	# for connected components count.
		...
		return sqlContext.createDataFrame(g.vertices.map(lambda x: (x.id, 1 if componentsCount(x.id, graphx) > connectedCount else 0)), ['id', 'articulation'])
